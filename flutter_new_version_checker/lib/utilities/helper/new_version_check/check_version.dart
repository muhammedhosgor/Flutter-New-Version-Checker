...
class NewVersionCheck {
  ...
  _checkPlayStore(String packageName) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        errorMsg =
        "Google Play Store'da kimliğe sahip bir uygulama bulunamıyor: $packageName";
      } else {
        //   print(response.body);
  ...
  _checkAppleStore(String packageName) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        errorMsg =
        "Apple Store'da kimliğe sahip bir uygulama bulamıyor: $packageName";
      } else {
  ...
  Future<void> openAnyUrl(String url) async {
    if (!await launchUrl(
      ...
      throw 'launch başlatılamadı $url';
    }
  }
}
